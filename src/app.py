...
default_genres = ["Action", "Adventure", "Comedy",
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    dcc.Loading([
                        ...
                        html.Div([
                            html.P("About:", style={
                                'text_aligh': 'left', 'color': 'charcol', 'font-family': 'sans-serif'}),
...
def plot_graphs(genre, year_range):
    ...
    filter_data = filter_genre.query(
        "Year >= @year_range[0] & Year <= @year_range[1] & Major_genre in @genre").dropna()
    ...
    bar = alt.Chart(filter_data, title='Gross Revenue By Genre (box office)').mark_bar().encode(
        x=alt.X('sum(US_Revenue)', axis=alt.Axis(
            title='Gross Revenue (in millions USD)')),
    ...
    line = alt.Chart(filter_data, title="Average Revenue By Genre (box office)").mark_line(point=True).encode(
        y=alt.Y("mean(US_Revenue)", axis=alt.Axis(
            title='Average Revenue (in millions USD)', format='$.0f')),
